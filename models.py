...
class User(UserMixin, db.Model):
    ...
    masked_aadhaar = db.Column(db.String(14))  # Format: XXXX-XXXX-1234
    aadhaar_verification_status = db.Column(db.String(20), default='pending')  # pending, verified, failed
    aadhaar_verification_id = db.Column(db.String(100))  # From KYC provider
    aadhaar_verified_at = db.Column(db.DateTime)
    
    # Company ID verification for recruiters
    company_id = db.Column(db.String(50))
    company_id_status = db.Column(db.String(20), default='pending')  # pending, verified, rejected
    ...
    # Security fields
    mfa_secret = db.Column(db.String(32))  # For admin MFA
    ...
    def is_aadhaar_verified(self):
        """Check if user has completed Aadhaar verification"""
        return self.aadhaar_verification_status == 'verified'
    
    def is_company_id_verified(self):
        """Check if recruiter has completed Company ID verification"""
        return self.company_id_status == 'verified'
    
    def is_fully_verified(self):
        """Check if user meets all verification requirements for their role"""
        if not self.is_aadhaar_verified():
            return False
        
        if self.role == 'recruiter':
            return self.is_company_id_verified()
        
        return True
    
    def can_login(self):
        """Check if user can login based on verification status"""
        if not self.active or self.locked_until and self.locked_until > datetime.utcnow():
            return False
    ...
    def mask_aadhaar(self, full_aadhaar):
        """Mask Aadhaar number for storage"""
        if len(full_aadhaar) >= 12:
            return f"XXXX-XXXX-{full_aadhaar[-4:]}"
        return None
